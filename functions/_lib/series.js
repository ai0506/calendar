...
export function seriesFromRequest(requestBody, id, idempotencyKey, now) {
  return {
    ...
    subject_id: requestBody.subject_id || null,
...
export function insertSeriesStatement(db, series) {
  return db.prepare(`INSERT INTO event_series
    (id, idempotency_key, title, description, category, subject_id, color, group_title,
    ...
    .bind(
      ...
      series.category, series.subject_id, series.color, series.group_title, series.all_day,
  ...
  return db.prepare(`INSERT INTO events
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(
      ...
      instance.end_time, series.all_day, series.category, series.subject_id, series.color,
