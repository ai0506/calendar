...
[ClaudeCode][260727163349] 修复 AI 通过 MCP 写入的事件 1ebae044-663e-41bb-beea-c1320d57593d（UXR课程加课）分类为不存在的 "UXR课程"：先把该事件 category 改回已注册的 "Research"；再治本——新增 functions/_lib/categories.js 的 ensureCategoryExists，接入全部 9 个写路径（events POST/PUT、events/import、event-series POST/PATCH、deadlines POST/PUT 及对应的 6 个 MCP 工具 runCreateEvent/runUpdateEvent/runCreateEventSeries/runUpdateEventSeries/runCreateDeadline/runUpdateDeadline），非法 category 一律拒绝为 validation_error（import 中计入 skipped）；同步更新 functions/mcp/index.js 的 category 字段描述（CATEGORY_DESCRIPTION，提示需先 calendar_list_categories 核对合法值，MCP 无建分类工具）、PROJECT_SPEC.md 与 API_DOC.md 说明该约束；单元测试与语法检查通过。
[ClaudeCode][260727163738] 补齐 category 校验改动在其余文档中的记录：CHANGELOG.md 的 Unreleased 新增 Fixed 条目；BUGS.md 新增 BUG-0005（已 Fixed）说明 AI Agent 曾借无校验的 category 字段写入未注册分类名 "UXR课程" 及修复方式；TEST_CHECKLIST.md 分类章节新增 9 条写路径的校验用例和一条待办的生产端到端验证。以上改动均为本地文件修改，尚未 commit / push / 部署。
[ClaudeCode][260727163748] file updated
[ClaudeCode][260727165920] 前端性能优化（public/app.js + index.html 缓存版本号）：事件/DDL 变多后月视图渲染卡顿、动画掉帧。1) trimMonthCells 改为「写/读/写」三段批处理并只取一次 padding，消除每个格子一次的强制回流（42 次 → 1 次）；2) 新增 measureChipCapacity，renderMonth 按当前格子高度只生成放得下的 chip（多余的记在 .events[data-dropped] 上并计入「+N more」），节点数 462 → 168，布局耗时减半；3) refreshVisibleData 增加数据指纹 dataSignature，10 秒轮询数据没变时不再重建 DOM；4) 新增 applyCachedRange，navigate/goToday/switchView 先套用预取缓存再渲染，翻页由 2~3 次全量渲染降为 1 次；5) 新增 holdRenders/render 延迟机制，swipe 与弹窗 spring 动画期间的后台渲染推迟到动画结束后合并执行；6) refreshVisibleData 用 pendingRefresh 补上「翻页时正好有请求在飞」会漏取新月份数据的问题，并在请求返回时校验区间未变；7) 页面切到后台时暂停 10s 轮询与时钟刷新，回到前台补一次强制刷新。实测 462 chip 的满月视图渲染 32ms → 10ms，且各视口高度下「+N more」与旧实现逐格一致。
[CodeX][260728194229] MCP OAuth 新增 calendar.read 最小权限：动态注册和授权码仅接受 calendar/calendar.read；read token 的 tools/list 仅返回查询工具，服务端拒绝写工具调用；保留旧 calendar 完整权限兼容，并新增 scope 行为测试和部署说明。
[ClaudeCode][260803143708] MCP 写入操作追加"是哪个 AI 改的"归因：新增迁移 0011_mcp_attribution.sql，给 events/deadlines/event_series 三张表加 last_modified_by 列；/oauth/token 签发 access token 时把 DCR 注册的 client_name（如 "Claude"）一并签进 token，避免 /mcp 每次请求都查库；functions/mcp/index.js 的 authenticate() 解出 clientName 并透传给全部 13 个写工具（events/deadlines 的 create/update/delete/complete、series 的 create/update/delete/skip/restore/split），每次写操作用一条 attributionStatement 把 last_modified_by 写回对应行；MCP_WRITE_TOKEN 本地调试旁路固定标记为 "local-debug"；REST/网页端 cookie 写入不受影响，该列继续为空。仅本地改动，未部署，需先 `npm run db:local`/`db:remote` 应用迁移。
[Codex][260805150824] Prepared migration handoff: added HANDOFF.md and FILE_TRANSFER_LIST.md, documented Git/local-state classification and new-computer recovery steps, and created a private archive plan that retains .dev.vars, local D1 state, production material, and optional Android signing files while excluding generated caches and logs.
